...
for test_case in range(1, T+1):
    ...
    index = []
    real_max = max(real_game_num)
    if real_game_num.count(real_max) >= 2:
        for k in range(len(real_game_num)):
            if real_game_num[k] == real_max:
                index.append(k)
    ...
    for ii in range(game_count[0]):
        for i in range(len(real_game_num)):
            for j in range(i, len(real_game_num)):
                ...
                if real_game_num[j] == real_max and real_game_num[i] == real_game_num_2[0] and real_game_num.count(real_max) >= 2 and real_game_num[i] < real_game_num[index[-1]]:
                    real_game_num[i], real_game_num[index[-1]] = real_game_num[index[-1]], real_game_num[i]
                    change_num = ''.join(real_game_num)
                    real_game_num[i], real_game_num[index[-1]] = real_game_num[index[-1]], real_game_num[i]
                elif real_game_num[j] == real_max and real_game_num[i] == real_game_num_2[1] and real_game_num.count(real_max) >= 2 and real_game_num[i] < real_game_num[index[-1]]:
